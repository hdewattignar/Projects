...
public static class MeshGenerator{
	...
	public static MeshData GenerateTerrainMesh(float[,] heightMap, float heighMapMultiplier, AnimationCurve heightCurve, int levelOfDetail)
    {
		...
		for (int y = 0; y < height; y += meshSimplificationIncrement)
        {
			for (int x = 0; x < width; x += meshSimplificationIncrement)
            {
				...
				meshData.verticies[vertexIndex] = new Vector3(topLeftX + x, heightCurve.Evaluate(heightMap[x,y]) * heighMapMultiplier, topLeftZ - y);
